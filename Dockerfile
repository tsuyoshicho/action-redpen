...
RUN apk --update add git curl jq wget && \
    ...
    rm /var/cache/apk/*
RUN wget -O - -q https://raw.githubusercontent.com/reviewdog/reviewdog/master/install.sh| sh -s -- -b /usr/local/bin/ ${REVIEWDOG_VERSION:7}
...
ENV PATH="/redpen-distribution-${REDPEN_VERSION:7}/bin:${PATH}"
